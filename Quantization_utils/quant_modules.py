...
class QuantLinear(Module):
    ...
    def forward(self, x, prev_act_scaling_factor=None, name="ne_inp_l1"):
        """
        ...
        return ste_round.apply(
            F.linear(x_int, weight=self.weight_integer, bias=self.bias_integer)) * correct_output_scale, torch.round(F.linear(x_int, weight=self.weight_integer, bias=self.bias_integer)), self.fc_scaling_factor
        ...
        if not self.full_precision_flag:
            ...
            # and the self.act_zero_point should be 0.
                ...
                self.act_scaling_factor, self.act_zero_point = asymmetric_linear_quantization_params(
                    self.activation_bit, torch.tensor([0.], device=self.device), self.x_max, True)
            if (pre_act_scaling_factor is None) or (self.fixed_point_quantization == True):
                ...
                for i in range(branch_num):
                    # I can re-use this part to write the separate quantization for the different scales.
...
class QuantAct_Concat(Module):
    """
    Class to quantize given activations for the concat layers
    ...
    def __init__(self,
                 ...
                 act_percentile=0,
                 fixed_point_quantization=False):
        super(QuantAct_Concat, self).__init__()
    ...
    def forward(self, x, pre_act_scaling_factor=None, pre_weight_scaling_factor=None, identity=None,
        ...
        """
        x: the activation that we need to quantize
        pre_act_scaling_factor: the list of scaling factors of the previous activation quantization layers before concat
        pre_weight_scaling_factor: the list of scaling factors of the previous weight quantization layers before concat
        identity: if True, we need to consider the identity branch
        identity_scaling_factor: the scaling factor of the previous activation quantization of identity
        identity_weight_scaling_factor: the scaling factor of the weight quantization layer in the identity branch
        ...
        if not self.full_precision_flag:
            ...
            # and the self.act_zero_point should be 0.
                ...
                self.act_scaling_factor, self.act_zero_point = asymmetric_linear_quantization_params(
                    self.activation_bit, torch.tensor([0.], device=self.device), self.x_max, True)
            if not isinstance(pre_act_scaling_factor, list):
                raise TypeError("pre_act_scaling_factor must be of type 'list'")
            if not isinstance(pre_weight_scaling_factor, list):
                raise TypeError("pre_weight_scaling_factor must be of type 'list'")
                # this is for the case of multi-branch quantization
            concats = len(pre_act_scaling_factor)
            quant_act_int = x
            partition = int(x.shape[1]/len(pre_act_scaling_factor)) # Assume equal dimension vector concat
            # start_channel_index = 0
            for i in range(concats):
                quant_act_int[:, i*partition: (i+1)*partition] \
                    = fixedpoint_fn.apply(x[:, i*partition: (i+1)*partition],
                                            self.activation_bit, self.quant_mode, self.act_scaling_factor, 0,
                                            pre_act_scaling_factor[i],
                                            pre_weight_scaling_factor[i])
                # start_channel_index += channel_num[i]
            correct_output_scale = self.act_scaling_factor.view(-1)
            return (quant_act_int * correct_output_scale, self.act_scaling_factor)
        else:
